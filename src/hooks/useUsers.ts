...
export function useUserStats(users: User[]): UserStats {
  // Filter out users with invalid createdAt values - had some issues with null dates
  const validUsers = users.filter(user => user.createdAt && typeof user.createdAt === 'string');
  
  return {
    totalUsers: users.length,
    usersPerDay: generateUsersPerDayData(validUsers),
    avatarDistribution: generateAvatarDistribution(users),
    signupTimeDistribution: generateSignupTimeDistribution(validUsers),
...
function generateUsersPerDayData(users: User[]) {
  // Generate array of last 30 days for the chart
  ...
  const userCounts = last30Days.map(date => {
    const count = users.filter(user => {
      if (!user.createdAt || typeof user.createdAt !== 'string') return false;
      try {
...
function generateSignupTimeDistribution(users: User[]) {
  ...
  users.forEach(user => {
    if (!user.createdAt || typeof user.createdAt !== 'string') return;
